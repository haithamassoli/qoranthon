import { addGameQuizMutation } from "@apis/quizzes";
...
const AddGameQuizScreen = () => {
  ...
  const { mutate, isLoading } = addGameQuizMutation();
  const onSubmit = (data: QuizValidationSchemaType) => {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          queryClient.invalidateQueries(["gameQuizzes"]);
          useStore.setState({
            snackbarText: "تم إضافة الغرفة بنجاح",
          });
          router.back();
        },
      }
...
export default AddGameQuizScreen;
